...
export default async function DashboardPage() {
  ...
  prospects.forEach((p) => {
    statusCounts[p.status as ProspectStatus] =
      (statusCounts[p.status as ProspectStatus] || 0) + 1;
  });

  const totalMessages = sequences.reduce(
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex h-4 overflow-hidden rounded-full">
              {(
                Object.entries(statusCounts) as [ProspectStatus, number][]
              )
                ...
                .map(([status, count]) => (
                  <div
                    ...
                    style={{
                      width: `${(count / prospects.length) * 100}%`,
                    }}
                    title={`${status}: ${count}`}
                  />
                ))}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {seq.style} &middot; {seq.messages?.length ?? 0} msgs
                      </p>
                    </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="mb-6 max-w-md text-center text-muted-foreground">
              Import your LinkedIn connections CSV, then generate personalized
              outreach sequences powered by AI.
            </p>
